...
# фильтр по режиссерам - по образу жанрового
        ...
        if str(god_ot) == 'None' or len(str(god_ot)) <= 2:
            ...
            god_filtr = " AND release_date BETWEEN '" + god_ot + \
                "-01-01' AND '" + god_do + "-12-31'"
        ...
        lim_step = " LIMIT " + str(paginacia) + " OFFSET " + \
            str(paginacia * (page - 1)) + ";"
...
# упаковываем в массив структуры(объекты класса извлеченного из БД)
        ...
        return render_template(
            'perechen.html',
            prch_th=prch_th,
            janre_list=janre_list,
            page=page,
            paginacia=paginacia,
            f_filtr_name=f_filtr_name,
            j_filtr_id=j_filtr_id,
            god_ot=god_ot,
            god_do=god_do,
            sortirovka=sortirovka,
            offset=offset,
            regis_list=regis_list,
            n_pages=n_pages,
            n_entr=n_entr,
            c_u=current_user
            )
...
#        connection.commit()
        ...
        connection.commit()
        cursor.close()
        logging.info(f"unit with ID={post_id} \
            sucsessfuly added by {current_user.name}")
...
#            print(os.path.join(app.config['UPLOAD_FOLDER'], new_filename))
        ...
        u_req = """
        UPDATE films
        SET film_name = %s, janre_id = %s, rejiser_id = %s,
        descript = %s, release_date = %s
        WHERE id = %s;
        """
        cursor = connection.cursor()
        ...
        connection.commit()
        logging.info(f"unit with ID={f_id} sucsessfuly edited \
        ...
        return render_template(
            'edited.html', result=result,
            c_u=current_user, f_n=f_n)
...
def deleting():
    if request.method == "GET":
        ...
        return render_template(
         'deleting.html', c_u=current_user,
        ...
        if f_control == 'Do it now!':
            ...
            logging.info(f"unit with ID={f_id}: IS NOT deleted \
...
# формируем запрос о таком пользователе
        ...
        if len(u_d) == 1:
            ...
            if str(d_passwd) == str(hash_passwd):
                # пароль правильный
